...
class A_Spider(scrapy.Spider):
    name = '30_b2'  # change this first!!!
    allowed_domains = ['inkcartridges']
    start_urls = [
        # WJINK1
        'https://www.inkcartridges.com/compatible-hasler-wjink1-red-ink-cartridge/'
    ]
    ...
    def parse(self, response):
        ...
        price = response.xpath(
            '//*[@id="product-price-51689"]/span/text()').extract_first()
        price = price[1:]
        print(price)

        # Reading Manufacturer
        producer = (response.xpath(
            '//*[@id="product-attribute-specs-table"]/tbody/tr[7]/td/text()').extract_first())
